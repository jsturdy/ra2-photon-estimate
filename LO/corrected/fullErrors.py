...
print "%6s%8s%8s%20s%20s"%("bin", "pred.", "stat", "syst. up/dn","total up/dn")
for i in range(len(zinvis_uncertainty_1)):
    pred   = zinvis_events[i]
    stat   = zinvis_uncertainty_1[i]
    systDN = zinvis_uncertaintyDN_2[i]
    systUP = zinvis_uncertaintyUP_2[i]

    totalUP = math.sqrt((stat*stat)+(systUP*systUP))
    totalDN = math.sqrt((stat*stat)+(systDN*systDN))
    if pred == 0:
        totalDN = 0.0
        
    print "%6s%8.2f%8.2f{%16.2f}_{-%2.2f}{+%16.2f}_{-%2.2f}"%(channel[i],
                                                     pred,
                                                     stat,
                                                     systUP,
                                                     systDN,
